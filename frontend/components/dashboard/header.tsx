...
import { Bell, Settings, User, ChevronDown, RefreshCw, LogOut, ShieldCheck } from 'lucide-react'
import { useState } from 'react'
import { useAuth } from '@/hooks/useAuth'

export function DashboardHeader() {
  const [showDropdown, setShowDropdown] = useState(false)
  const { profile, currentRole, setCurrentRole, logout } = useAuth()

  const roles = [
    { id: 'contestant', label: 'Contestant', icon: User },
    { id: 'voter', label: 'Voter', icon: RefreshCw },
    { id: 'admin', label: 'Administrator', icon: ShieldCheck, restricted: true }
  ]

  const activeRoles = roles.filter(r => !r.restricted || profile?.role === 'admin')
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-black transition-all ${
                  isActive 
                    ? 'bg-zed-primary text-white shadow-[0_2px_10px_rgba(79,70,229,0.4)]' 
                    : 'text-zed-foreground-secondary hover:text-zed-foreground'
                }`}
              >
                <Icon size={12} />
                {role.label}
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-3 px-3 py-1.5 rounded-xl hover:bg-white/5 transition-all click-push"
          >
            ...
            <div className="hidden sm:flex flex-col items-start leading-none">
              <span className="text-sm font-black tracking-tight">{profile?.fullName || 'Arena User'}</span>
            ...
            <div className="absolute top-full right-0 mt-3 w-56 glass-premium border-white/10 rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.5)] z-50 animate-zed-fade-up">
              <div className="p-4 space-y-2">
                <div className="flex items-center gap-3 pb-4 border-b border-white/10">
                  ...
                  <div>
                    <div className="text-sm font-black text-zed-foreground truncate w-32">{profile?.fullName || 'Arena User'}</div>
                ...
                <button 
                  onClick={logout}
                  className="w-full flex items-center gap-3 px-4 py-2.5 text-xs font-bold hover:bg-red-500/10 rounded-xl transition-all text-red-400"
                >
                  <LogOut size={14} />
                  Exit Arena
                </button>
