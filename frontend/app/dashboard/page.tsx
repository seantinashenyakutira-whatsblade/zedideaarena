...
export default function DashboardPage() {
  ...
  const stats = [
    ...
    { icon: CheckCircle, label: 'Published', value: ideas.filter(i => i.status === 'submitted').length.toString(), color: 'text-zed-success' },
  ]
          ...
          <div className="container-zed py-8">
            ...
            <div className="mb-8">
              <div className="flex items-center justify-between mb-6">
                ...
                <Link href="/dashboard/ideas/new" className="btn-primary text-sm">
                  New Idea
                </Link>
              </div>
                      ...
                      <tbody>
                        {ideas.map((idea: any) => (
                          <tr key={idea.id} className="border-b border-zed-border hover:bg-zed-surface/50 transition-colors">
                            ...
                            <td className="px-6 py-4">
                              <span
                                className={`inline-block px-3 py-1 rounded-zed-pill text-xs font-bold ${
                                  idea.status === 'submitted'
                                    ? 'bg-zed-success/20 text-zed-success'
                                    : 'bg-yellow-500/20 text-yellow-400'
                                }`}
                              >
                                {idea.status === 'submitted' ? 'Published' : 'Draft'}
                              </span>
                            </td>
                            <td className="px-6 py-4 text-zed-foreground">{idea.votes || 0}</td>
                            <td className="px-6 py-4">
                              <button className="text-zed-primary hover:text-zed-accent transition-colors">
                                <ArrowRight size={20} />
                              </button>
                            </td>
                          </tr>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="card-zed-hover cursor-pointer">
                ...
                <p className="text-sm text-zed-foreground-secondary mb-4">
                  Got a brilliant idea? Submit it to one of our active competitions and compete for funding.
                </p>
              ...
              <div className="card-zed-hover cursor-pointer">
                ...
                <p className="text-sm text-zed-foreground-secondary mb-4">
                  Browse ideas from creators worldwide. Vote for the ones you believe in and help shape the future.
                </p>
