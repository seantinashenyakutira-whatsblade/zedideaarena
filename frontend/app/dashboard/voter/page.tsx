  ...
  const steps = [
    {
      ...
      desc: 'Secure KYC powered by Didit.me',
      icon: ShieldCheck,
      status: profile?.kyc_status === 'verified' ? 'completed' : profile?.kyc_status === 'pending' ? 'pending' : 'todo',
      link: '/dashboard/kyc'
    },
    {
      ...
      icon: CreditCard,
      status: profile?.voter_payment_status === 'paid' ? 'completed' : 'todo',
    ...
    },
    {
      ...
      icon: Vote,
      status: (profile?.kyc_status === 'verified' && profile?.voter_payment_status === 'paid') ? 'todo' : 'locked',
            ...
            <div className="max-w-4xl mx-auto">
              <div className="mb-12 text-center md:text-left">
                ...
                <p className="text-zed-foreground-secondary text-lg max-w-2xl">
                  To ensure the integrity of the Arena, all voters must verify their identity and pay a seasonal participation fee.
                </p>
              </div>
              ...
              <div className="grid md:grid-cols-3 gap-6">
                {steps.map((step, i) => {
                  ...
                  return (
                    <div 
                      key={i}
                      className={`card-zed relative p-8 flex flex-col gap-6 transition-all duration-300 ${
                        isLocked ? 'opacity-50 grayscale pointer-events-none' : 'hover:scale-[1.02]'
                      } ${isCompleted ? 'border-zed-success/30' : ''}`}
                    >
                      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${
                        isCompleted ? 'bg-zed-success/10 text-zed-success' : 'bg-zed-primary/10 text-zed-primary'
                      }`}>
                        <Icon size={24} />
                      </div>
                      ...
                      <div className="mt-auto">
                        {isCompleted ? (
                          ...
                          <Link 
                            href={step.link}
                            className="flex items-center justify-between w-full btn-primary py-3 px-4 text-xs font-black rounded-xl"
                          >
                ...
                <div className="mt-12 p-8 bg-white/5 rounded-3xl border border-white/5 flex items-start gap-6">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-zed-foreground-secondary leading-relaxed max-w-xl">
                      ZedIdeaArena uses identity verification and a small entry fee to prevent sybil attacks, bot manipulation, and ensure that every vote comes from a real, committed person. This maintains the fairness of the competition and the value of the funding prizes.
                    </p>
                  </div>
                </div>
              )}
